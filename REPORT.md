...
**Nền tảng:** Web Application (React + TypeScript + Vite) hỗ trợ PWA.
...
| **PWA** | **Vite PWA Plugin** | Hỗ trợ cài đặt ứng dụng (Installable), hoạt động Offline (Service Workers). |
...
*   **`vite.config.ts`**: Cấu hình Vite Server, Plugin React và **VitePWA** (Manifest, Workbox strategy).
...
*   *Lưu ý: Module âm thanh đã được loại bỏ để tối ưu hóa trải nghiệm thị giác.*

### 3.4. Thành phần Giao diện (Components)
*   **`BreathingGuide.tsx`**: Màn hình thực hành chính. Tích hợp Mood Tracker (Trước/Sau).
...
1.  **Khởi động:**
    ...
    *   Service Worker đăng ký (nếu môi trường Production) để cache assets.
    *   Tải dữ liệu tiến trình từ IndexedDB.

2.  **Tập luyện:**
    *   **Bước 1: Mood Check-in:** Người dùng chọn trạng thái cảm xúc hiện tại.
    *   **Bước 2: Practice:** Vòng tròn thở với hiệu ứng Animation mượt mà.
    *   **Bước 3: Mood Check-out:** Đánh giá lại cảm xúc.
...
*   **Tính năng mới (New Features):**
    *   **PWA (Progressive Web App):**
        *   Cho phép cài đặt ứng dụng lên màn hình chính (Mobile/Desktop).
        *   Hỗ trợ hoạt động ngoại tuyến (Offline support).
        *   Đồng bộ màu thanh trạng thái (Theme color) theo chế độ Sáng/Tối.
...
*   **Thay đổi hệ thống (System Changes):**
    *   **Loại bỏ Audio:** Tập trung vào trải nghiệm Visual Zen.
    *   **Xóa Proposal:** Hoàn tất giai đoạn đề xuất, chuyển sang giai đoạn duy trì và tối ưu.
